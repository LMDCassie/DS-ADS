...
int main() {
	...
	for (n = 256; n <= 65536; n *= 2) { // Search和Delete的测试中，这里大致相同但略有差别，n是SkipList的大致规模
		int TotalTime = 0;
		for (int test = 0; test < 10; test++) {// Search和Delete的测试中，这里大致相同但略有差别，test和i的数值 关系到操作总共进行了多少次
			srand(time(NULL));
			...
			for (i = 0; i < n - 100; i++) InsertNode(skl, value[i], key[i]); //先建好一个一定规模的SikpList 再进行插入操作

			clock_t start, finish;
			start = clock();
			for (i = n - 100; i < n; i++) { // Search和Delete的测试中，这里大致相同但略有差别，test和i的数值 关系到操作总共进行了多少次
				InsertNode(skl, value[i], key[i]);
			}//连续插入操作一百次
			finish = clock();
			TotalTime += finish - start;
			delete skl;
		}
		//不同规模的SkipList 测试1000次插入操作，输出时间测试结果
